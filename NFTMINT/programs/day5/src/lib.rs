...
pub mod Nftescrow{
    ...
    //listing back the nft to the seller

    pub fn Listbacknft(ctx:Context<Liskingback>)->Result<()>{
        let escrow=&ctx.accounts.escrow;

        require!(!escrow.is_sold,Escrowerror::AlreadySold);
...
//adding the revert back from the listing 

#[derive(Accounts)]
pub struct Liskingback<'info>{
    #[account(mut,seeds=[b"nftescrow",seller.key().as_ref(),mint.key().as_ref()],bump=escrow.bump,close=seller)]
    pub escrow:Account<'info,NftEscrow>,
    #[account(mut)]
    pub seller:Signer<'info>,
